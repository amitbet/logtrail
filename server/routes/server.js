...
function getValFromSource(source, fieldOrArr) {

  if (fieldOrArr.indexOf(",") !== -1) {
    fieldOrArr = fieldOrArr.split(",");
    //console.log("got array - fieldOrArr: " + JSON.stringify(fieldOrArr));
    //if (Array.isArray(fieldOrArr)) {
    for (var i = 0; i < fieldOrArr.length; i++) {
      var field = fieldOrArr[i];
      if (source[field])
        return source[field];
    }
  } else {
    return source[fieldOrArr];
  }
}
...
function convertToClientFormat(selected_config, esResponse) {
  ...
  for (var i = 0; i < hits.length; i++) {
    ...
    event['display_timestamp'] = getValFromSource(source, selected_config.fields.mapping['display_timestamp']);
    event['timestamp'] = getValFromSource(source, selected_config.fields.mapping['timestamp']);
    event['hostname'] = getValFromSource(source, selected_config.fields.mapping['hostname']);
    event['message'] = getValFromSource(source, selected_config.fields.mapping['message']);
    event['program'] = getValFromSource(source, selected_config.fields.mapping['program']);

    if (selected_config.fields.mapping['level'])
      event['level'] = getValFromSource(source, selected_config.fields.mapping['level']);

    if (selected_config.fields.mapping['env'])
      event['env'] = getValFromSource(source, selected_config.fields.mapping['env']);
